...
cell_types <- unique(selected_impute_sce$X2017_WHO)
...
for(c in cell_types){
  each_sce <- selected_impute_sce[,selected_impute_sce$X2017_WHO == c]
...
source("~/Desktop/EPN_metabolism/utils.R")
...
scran.sf <- scran::calculateSumFactors(selected_impute_counts[low_dropout_genes,],clusters=selected_impute_sce$X2017_WHO)
summary(scran.sf)
...
all_cell_type <- as.vector(selected_impute_sce$X2017_WHO)
